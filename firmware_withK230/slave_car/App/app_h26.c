...
{
    /* This is the established stop path for both DC and stepper configurations. */
...
static void H26_StartCurrentTask(void)
{
    ...
    {
        /* Requirements 3--6 are explicit safe placeholders in this revision. */
        H26_ClearRunData();
        s_systemState = H26_SYS_SELECT;
...
void H26_KeyProcess(void)
{
    ...
    if (key == 3U)
    {
        /* K3 is accepted from every state; this also makes a stopped vehicle safe. */
        H26_SafeStop();
...
void H26_Task10ms(void)
{
    ...
    case H26_SYS_PREPARE:
        /* Data was cleared synchronously with K2; this keeps preparation motor-safe. */
        H26_SafeStop();
...
void H26_Task100ms(void)
{
    ...
    elapsedMs = H26_Task2_GetElapsedMs(Timer_GetMs());
...
{
#if CAR_OLED_ENABLE
    uint32_t elapsedMs = H26_Task2_GetElapsedMs(Timer_GetMs());
